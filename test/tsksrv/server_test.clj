...
;; ----------------------------------------------------
(against-background [(before :contents (srv/start!))
                     (after :contents (srv/stop!))]
...
(fact "The version is served at the root "
      (let [port (conf/config :port)
            ...
            resp (client/get (str "http://localhost:" port "/ping"))]
        (nil? resp) => false
        (:status resp) => 200
        (:body resp) => "0.0.1")))
